...
class light_control(appapi.my_appapi):
  ...
  def initialize(self):
    ...
    self.control_dict={"light.den_fan_light":{"light.den_fan_light":{"type":"dimmer","on":"50","off":"0","last_brightness":""}},
                       ...
                       "media_player.dentv":{"light.den_fan_light":{"type":"dimmer","on":"10","off":"last","last_brightness":""},
                                             "switch.breakfast_room_light":{"type":"switch","on":"off","off":"last"}},
  ...
  def set_light_state(self,entity,light,entity_state):
    ...
    elif self.control_dict[entity][light][entity_state]=="dim":      # dim light till it's off
      self.log("dim light {} till it's off".format(light))
      if self.get_state(light,attribute="state")=="on":              # check if the light is on
        self.turn_on(light,brightness=255)
        self.run_in(self.dim_light,60,light=light)                 # schedule a call back to dim the light some
    else:
      self.log("New desired state for {} is on or has a dimmmer value {}".format(light,self.control_dict[entity][light][entity_state]))
      if self.get_state(light,attribute="state")=="on":
        ...
        if self.control_dict[entity][light]["type"]=="dimmer":     # light is a dimmer so save current state and adjust to new value
          self.log("{} is a currently on dimmer seting value to {} ".format(light,self.control_dict[entity][light][entity_state]))
          self.control_dict[entity][light]["last_brightness"]=self.get_state(light,attribute="brightness")
          if self.control_dict[entity][light][entity_state]=="dim":
            self.turn_on(light,brightness=255)
          else:
  ...
  def dim_light(self,kwargs):
    ...
    if self.get_state(light)=="on":
      ...
      if not ((self.get_state("input_boolean.spot")=="on") and (light in spot_lights)):   # are we dealing with an exception (spot cleaning)
